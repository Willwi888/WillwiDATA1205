...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // If embedded in Carrd, we want transparent backgrounds to let Carrd's design show through
  ...
  return (
    ...
    <div className={`min-h-screen flex flex-col relative font-sans selection:bg-brand-accent selection:text-brand-darker text-slate-100 overflow-x-hidden ${isEmbed ? 'bg-transparent' : 'bg-slate-950'}`}>
      ...
      {/* Only render the heavy background system if NOT in embed mode. 
          In Embed mode, we rely on Carrd's background. */}
      {!isEmbed && (
      ...
      {/* --- PREMIUM NAVBAR --- */}
      {/* In Embed Mode, we might want a simpler navbar or keep it transparent */}
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-slate-950/95 backdrop-blur-2xl border-b border-slate-800 absolute w-full left-0 top-full shadow-2xl animate-fade-in-down">
            <div className="px-4 pt-4 pb-6 space-y-2">
              <Link to={isEmbed ? "/?embed=true" : "/"} onClick={() => setIsMenuOpen(false)} className={mobileLinkClass('/')}>{t('nav_home')}</Link>
              <Link to={isEmbed ? "/database?embed=true" : "/database"} onClick={() => setIsMenuOpen(false)} className={mobileLinkClass('/database')}>{t('nav_catalog')}</Link>
              <Link to={isEmbed ? "/interactive?embed=true" : "/interactive"} onClick={() => setIsMenuOpen(false)} className={mobileLinkClass('/interactive')}>{t('nav_interactive')}</Link>
              <Link to={isEmbed ? "/add?embed=true" : "/add"} onClick={() => setIsMenuOpen(false)} className="block px-3 py-3 mt-4 text-center rounded-sm bg-brand-accent text-slate-950 font-bold uppercase tracking-widest">
                + {t('nav_add')}
              </Link>
            </div>
          </div>
      ...
      {/* --- PREMIUM FOOTER --- */}
      {/* Hide Footer in Embed Mode to save vertical space/redundancy with Carrd footer */}
      {!isEmbed && (
            ...
            <div className="max-w-7xl mx-auto pt-16 pb-8 px-6 lg:px-8">
                ...
                <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <Link to={isEmbed ? "/admin?embed=true" : "/admin"} className="text-[10px] text-slate-700 hover:text-slate-400 uppercase tracking-widest transition-colors">Manager Login</Link>
                    </div>
                </div>
            </div>
        </footer>
