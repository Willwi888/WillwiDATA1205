    ...
    <div className="max-w-screen-2xl mx-auto px-10 pt-48 pb-60 animate-fade-in">
      ...
      <div className="mb-24 space-y-16">
          <div className="flex items-end justify-between border-b border-white/5 pb-12">
            ...
            <div className="text-right">
               <span className="text-[10px] uppercase tracking-[1em] text-white/20 block mb-2">Validated Records</span>
          ...
          <div className="flex flex-col md:flex-row items-center gap-12">
            <div className="flex-1 w-full border-b border-white/10">
                <input
                  ...
                  className="w-full bg-transparent py-4 text-white outline-none text-2xl font-thin uppercase tracking-tight placeholder:text-white/5 focus:placeholder:text-white/20 transition-all"
                  value={searchTerm}
                ...
                <div className="flex gap-4">
                    {['All', ...Object.values(Language)].map(l => (
                        <button 
                            ...
                            className={`text-[9px] uppercase tracking-widest px-4 py-1 border transition-all whitespace-nowrap ${filterLang === l ? 'border-brand-gold text-brand-gold' : 'border-white/5 text-white/20 hover:text-white hover:border-white/20'}`}
                        >
                            {l}
                        </button>
                    ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-x-10 gap-y-20">
          {groupedAlbums.map(albumSongs => {
              ...
              return (
                  <div key={main.id} onClick={() => navigate(`/song/${main.id}`)} className="group cursor-pointer animate-fade-in">
                      ...
                      <div className="mt-8 space-y-2">
                          ...
                          <div className="flex items-center gap-4 text-[9px] font-mono text-white/20">
                              ...
                              <span>•</span>
                              <span className="truncate">{main.upc || 'NO_UPC'}</span>
                          </div>
                      </div>
                  </div>
              );
