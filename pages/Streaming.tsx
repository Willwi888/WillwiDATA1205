...
const Streaming: React.FC = () => {
  ...
  const featuredPlaylists = [
    { id: 'mv', title: 'Music Videos', count: 'Official MV', desc: '高品質視覺影像與故事呈現。', icon: '🎬' },
    { id: 'live', title: 'Live Performance', count: 'Live Session', desc: '現場演出的純粹能量。', icon: '🎤' },
    { id: 'vlog', title: 'Behind Scenes', count: 'Studio Vlog', desc: '創作幕後錄音與製作紀實。', icon: '📹' },
    { id: 'shorts', title: 'Short Clips', count: 'Exclusive Clips', desc: '官網限定短篇創作片段。', icon: '⚡' }
  ];
  ...
  return (
    <div className="animate-fade-in max-w-7xl mx-auto px-6 pt-12 pb-40">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        ...
        {/* LEFT: YouTube Exclusive Zone (7/12 width) */}
        <div className="lg:col-span-7 space-y-12">
          ...
          {/* Featured Content: Primary Embed */}
            ...
            <div className="relative group bg-slate-900 border border-white/10 overflow-hidden rounded-2xl shadow-2xl transition-all duration-700 hover:border-red-600/40">
              ...
              <div className="p-10 bg-gradient-to-b from-slate-900 to-black">
                ...
                <p className="text-slate-400 text-sm leading-relaxed mb-10 max-w-xl font-light">
                  探索 Willwi 為官網聽眾準備的獨家視聽內容。直接在此觀看最新 MV，或訂閱官方頻道以獲得更多第一手創作動態。
                </p>
          ...
          <div className="space-y-6 pt-12 border-t border-white/5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {featuredPlaylists.map((item) => (
                <a 
                  key={item.id} 
                  href={`https://www.youtube.com/@Willwi888/playlists`} 
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="p-8 bg-white/[0.02] border border-white/5 rounded-xl hover:bg-white/[0.05] hover:border-red-600/30 transition-all group flex flex-col h-full"
                >
                  <div className="flex justify-between items-start mb-4">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 group-hover:text-red-500 transition-colors">
                      {item.count}
                    </span>
                  ...
                  <div className="mt-4 pt-4 border-t border-white/5 flex items-center gap-2 text-[9px] font-bold text-slate-600 uppercase tracking-widest group-hover:text-white transition-colors">
                    Browse Collection
                    <svg className="w-3 h-3 transition-transform group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
                  </div>
                </a>
              ))}
            </div>
        ...
        {/* RIGHT: Spotify Audition Hub (5/12 width) */}
        <div className="lg:col-span-5 space-y-12">
          ...
          <div className="space-y-4 pt-6">
            <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 text-center">Other Streaming Platforms</div>
