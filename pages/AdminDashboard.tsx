    ...
    <div className="max-w-[1600px] mx-auto px-10 py-48 animate-fade-in pb-40">
      ...
      {(isSyncing || localImporting) && (
        <div className="fixed top-0 left-0 w-full z-[1000]">
           ...
           <div className="bg-brand-gold text-black text-[9px] font-black px-4 py-1.5 uppercase tracking-widest inline-block shadow-xl">
              Processing: {syncProgress}%
           </div>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-24 gap-10">
        <div>
          ...
          <p className="text-brand-gold text-[11px] font-black uppercase tracking-[0.6em] mt-4">
            {isSyncing ? `同步中 ${syncProgress}%` : 'Pure Data Management'}
          </p>
        </div>
      ...
      {activeTab === 'catalog' && (
        <div className="space-y-12 animate-fade-in">
          {showSpotifySearch && (
            <div className="bg-emerald-950/20 border border-emerald-500/20 p-10 space-y-8 animate-fade-in-up mb-12">
                  ...
                  {spotifyResults.map(track => (
                    <div key={track.id} onClick={() => toggleSpotifySelection(track.id)} className={`flex items-center gap-6 p-4 border transition-all cursor-pointer ${selectedSpotifyIds.has(track.id) ? 'bg-emerald-500/20 border-emerald-500' : 'bg-black/40 border-white/5'}`}>
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[9px] text-slate-500 uppercase">{track.album.name}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {selectedSpotifyIds.size > 0 && (
                <button onClick={handleBulkImportSpotify} className="w-full py-5 bg-emerald-500 text-black font-black uppercase text-xs tracking-widest">
          ...
          <div className="relative">
            <input type="text" placeholder="SEARCH CATALOG..." className="w-full bg-transparent border-b border-white/10 px-0 py-8 text-2xl outline-none focus:border-brand-gold text-white font-bold uppercase tracking-widest" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
          ...
          <div className="space-y-8">
            {groupedAlbums.map((album) => {
              ...
              return (
                <div key={main.id} className="border border-white/5 bg-white/[0.01] overflow-hidden">
                  <div 
                    onClick={() => toggleAlbum(main.id)}
                    className="flex items-center gap-8 p-6 cursor-pointer hover:bg-white/[0.03] transition-all"
                  >
                    <img src={main.coverUrl} className="w-20 h-20 object-cover shadow-2xl" />
                    <div className="flex-1">
                      <h4 className="text-xl font-black text-white uppercase tracking-wider">{main.title}</h4>
                      <div className="flex gap-6 mt-1 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                        <span>{album.length} 首歌</span>
                        <span>{main.releaseDate}</span>
                        {main.upc && <span>UPC: {main.upc}</span>}
                      </div>
                    </div>
                    <div className="flex items-center gap-6">
                       <span className={`text-xl transition-transform duration-500 ${isExpanded ? 'rotate-180' : ''}`}>↓</span>
                    </div>
                  </div>
                  ...
                  {isExpanded && (
                    <div className="border-t border-white/5 bg-black/40 animate-fade-in">
                      {album.map((track) => (
                        <div key={track.id} className={`flex items-center gap-8 p-6 border-b border-white/5 last:border-0 hover:bg-brand-gold/5 transition-all ${currentSong?.id === track.id ? 'bg-brand-gold/10' : ''}`}>
                          <button 
                            onClick={(e) => handlePlayTrack(e, track)}
                            className={`w-10 h-10 rounded-full border border-white/20 flex items-center justify-center transition-all ${isPlaying && currentSong?.id === track.id ? 'bg-brand-gold text-black border-brand-gold shadow-glow' : 'hover:border-brand-gold hover:text-brand-gold'}`}
                          >
                          ...
                          <div className="flex-1">
                            <p className="text-sm font-bold text-white uppercase">{track.title}</p>
                          ...
                          <div className="flex items-center gap-6">
                            <button 
                              ...
                              className={`px-3 py-1 text-[9px] font-black uppercase tracking-widest border ${track.isInteractiveActive ? 'bg-emerald-500/20 text-emerald-500 border-emerald-500/40' : 'text-slate-600 border-white/10'}`}
                            >
                            ...
                            <button onClick={() => { if (confirm('確定要刪除嗎？')) deleteSong(track.id); }} className="text-[10px] font-black uppercase text-rose-900 hover:text-rose-500 transition-all">刪除</button>
                          </div>
                        </div>
                      ))}
                    </div>
